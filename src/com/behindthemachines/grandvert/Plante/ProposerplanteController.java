 ...
 * @author ahmed
 */
public class ProposerplanteController implements Initializable {
    ...
    private void handleradd(ActionEvent event) {
       ...
       if(dis)Alertbox.display("Erreur", "le nom de la plante "+nom.getText()+" existe");
        ...
        {   if(Integer.parseInt(stock.getText())<0)Alertbox.display("Erreur", "le stock doit étre positive");
         ...
         ps.add(new Plante(nom.getText(),photo,desc.getText(), Integer.parseInt(stock.getText()), Float.parseFloat(prix.getText()), Integer.parseInt(hauteur.getText()), fertiliseur.getText(), cat.getValue(), season.getValue(),1,StageManager.getStageManager().getUser()));
